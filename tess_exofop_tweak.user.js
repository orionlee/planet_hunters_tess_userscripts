...
function tweakGotoCidMultiResult() {
  ...
  function propagateHash() {
    ...
    document
      .querySelectorAll('table.tfop tr > td:nth-of-type(1) a')
      .forEach((a) => {
...
function tweakNoTicFoundResult() {
  let [, id] = location.href.match(/target[.]php[?]id=([^&#]+)/) || [
    null,
    null,
  ];
  if (!id) {
  ...
  const maybeErrMsg = document.querySelector('a[name="top"] + table')
...
function bjtdToRelative(tBjtd) {
  ...
  const sectorStartStopList = [
    ...
    [91, 3775.296431629427, 3802.748268399669],
    [92, 3802.96769460401, 3829.639451903132],
    [93, 3829.875231202462, 3856.033503070185],
    [94, 3856.264693450251, 3882.118925669928],
    ...
    [106, 4233.429416851645, 4261.875729892523],
...
function getAliasesList() {
  let aliasesText = document.querySelector(
    '.overview_header > span:nth-of-type(1) > ul >li:nth-of-type(1)',
  )?.textContent;
  if (!aliasesText) {
...
function getDistance() {
  ...
  const headerText = document
    .querySelector(
      '#myGrid4 div.ag-header-container > div:nth-of-type(1) > div:nth-of-type(18)',
    )
    ?.textContent?.trim();
  if (headerText != 'Distance (pc)') {
    console.warn(
      'getDistance(): the identified column is not distance. Actual: ' +
        headerText,
    );
    return '';
  }
  ...
  const res = document.querySelector(
    '#myGrid4 div.ag-center-cols-container > div:nth-of-type(1) > div:nth-of-type(18)',
  )?.textContent;
  return res || '';
} // function getDistance()
...
function getBandMagnitudeMap() {
  const rowEls = Array.from(
    document.querySelectorAll(`#myGrid6 div.ag-center-cols-container > div`),
  );
  const res = {}; // band: magnitude map
  rowEls.forEach((rowEl) => {
    // round the magnitude for ease of visual matching
    res[rowEl.querySelector('div:nth-child(1)').textContent] = parseFloat(
      rowEl.querySelector('div:nth-child(2)').textContent,
    )?.toFixed(2);
  });
  return res;
}
  ...
  function getProperMotion() {
    ...
    let pmText =
      document.querySelector(
        '.overview_header > span:nth-of-type(2) > ul > li:last-of-type table tr:first-of-type',
      )?.textContent || '';
...
function getCoord() {
  ...
  let raDecEl = document.querySelector(
    '.overview_header > span:nth-of-type(2) > ul > li:nth-of-type(1)',
  );
  let coordUiCtrIdx = 2;
  if (!raDecEl) {
    raDecEl = document.querySelector(
      '.overview_header > span:nth-of-type(3) > ul > li:nth-of-type(1)',
    );
    coordUiCtrIdx = 3;
  }
...
function addExternalLInks() {
  ...
  if (simbadLinkEl) {
    ...
    if (simmbadHost) {
      simbadLinkEl.href = simbadLinkEl.href.replace(
        /^https:\/\/[^/]+/,
        `https://${simmbadHost}`,
      );
    }
  ...
  const vsxUrl =
    ...
    (coord != null
      ? `#coord=${encodeURIComponent(coord.ra + ' ' + coord.dec)}`
      : '');
  ...
  let gaiaDr3VarUrl =
    `https://${vizierHost}/viz-bin/VizieR-3?-source=+I%2F358%2Fvarisum+I%2F358%2Fvclassre+I%2F358%2Fveb+I%2F358%2Fvcc+I%2F358%2Fvst` +
    (coord != null
      ? `&-c=${encodeURIComponent(coord.ra + ' ' + coord.dec)}&-c.r=60&-c.u=arcsec#autoSubmit=true`
      : '');
  ...
  let gaiaDr3Url =
    `https://${vizierHost}/viz-bin/VizieR-3?-source=I/355/gaiadr3&-source=I/355/paramp` +
    my_GM_getValue('gaiaDr3UrlExtraParams', '') +
    (coord != null
      ? `&-c=${encodeURIComponent(coord.ra + ' ' + coord.dec)}&-c.r=15&-c.u=arcsec#autoSubmit=true`
      : '');
  gaiaDr3Url = appendGaiaIdForMatch(gaiaDr3Url);

  // Gaia DR3 Non Single Star
  const coordStrEncoded =
    coord != null ? encodeURIComponent(coord.ra + ' ' + coord.dec) : '';
  let gaiaDr3NSSUrl = `https://${vizierHost}/viz-bin/VizieR-4?-source=I/357&-c=${coordStrEncoded}&-c.u=arcsec&-c.r=15&-c.eq=J2000&-out.add=_r&-out.add=_p&-sort=_r`;
  ...
  let gaiaDr3XmatchVarUrl = `https://${vizierHost}/viz-bin/VizieR-4?-source=J/A%2bA/674/A22/catalog&-c=${coordStrEncoded}&-c.u=arcsec&-c.r=120&-c.eq=J2000&-c.geom=r&-out.add=_r&-out.add=_p&-sort=_r&-out=GaiaDR3&-out=ONames&-out=RAJ2000&-out=DEJ2000&-out=Omags&-out=psuperclass&psuperclass=%21%3D%2CCST%2CAGN%2CGALAXY&-out=pvarTypes&-out=VarTypes&-out=VarTypesOri&-out=AltVarTypesOri&-out=pPer&-out=OPers&-out=ORefEpoch&-out=Cats&-out=Sel`;
  ...
  let gaiaDr3StellarVarUrl = `https://${vizierHost}/viz-bin/VizieR-4?-out.add=_r&-out.add=_p&%2F%2Foutaddvalue=default&-sort=_r&-order=I&-c=${coordStrEncoded}&-c.eq=J2000&-c.r=+30&-c.u=arcsec&-c.geom=r&-source=&-out.src=J%2FA%2BA%2F677%2FA137%2Fcatalog&-source=J%2FA%2BA%2F677%2FA137%2Fcatalog&-bmark=GET`;
  ...
  const tceUrl = (() => {
    ...
    const urlPrefix = my_GM_getValue(
      'tceUrlPrefix',
      'https://exo.mast.stsci.edu/#search=TIC ',
    );
  ...
  const lcViewerUrl = (() => {
    ...
    const urlPrefix = my_GM_getValue(
      'lcViewerUrlPrefix',
      'https://tess-lightcurves.streamlit.app/?tic=',
    );
    return `${urlPrefix}${tic}`;
  })();
  ...
  const kic = (() => {
    const kicAliases = getAliasesList().filter((a) => a.startsWith('KIC '));
    return kicAliases.length > 0 ? kicAliases[0].substr(4) : null;
  })();
  let keplerLinkHTML = '';
  if (kic) {
    const keplerUrl = `https://archive.stsci.edu/kepler/data_search/search.php?action=Search&ktc_kepler_id=${kic}`;
    keplerLinkHTML = `  <a href="${keplerUrl}" target="_kepler" title="Kepler LC and DVs">Kepler</a> |`;
  }
  ...
  document.querySelector('a[href="/tess"]').insertAdjacentHTML(
    'afterend',
    `\
...
${keplerLinkHTML}
  ...
  Array.from(document.querySelectorAll('a[target="simbad"]'), (a) => {
    let href = a.href;
  ...
  Array.from(
    document.querySelectorAll('a[target="_vsx"], a[target="_asas-sn"]'),
    (a) => (a.href += hashToAdd),
  );
} // function addExternalLinks()
addExternalLInks();
...
function tweakMag() {
  ...
  const anchorEl = document.querySelector(
    '.overview_header > span:nth-child(1) > ul > li:nth-child(3)',
  );
  ...
  const colorIndexHTML = (() => {
    ...
    const bvColorIndexStr = isNaN(bvColorIndex)
      ? 'N/A'
      : bvColorIndex.toFixed(2);
...
//
(() => {
  const observationNoteCtr = document.querySelector(
    ".overview_header0 a[href^='edit_obsnotes.php']",
  );
  if (!observationNoteCtr) {
    console.warn(
      'Observation Note highlight failed: cannot find the element.  No-Op.',
    );
    return;
  }
...
(() => {
  ...
  if (maxNumTOIorCTOIs > 0) {
    // if any TOI/CTOI, indicate it in title
    console.debug('tweak title');
    document.title = `(${maxNumTOIorCTOIs}) ExoFOP TIC ${getTic()}`;
  }

  // highlight the existence of stellar companion
  const numCompanions = parseInt(
    document.querySelector('a[href="#stellarcomp"] > span')?.textContent,
    10,
  );
  if (numCompanions > 0) {
  ...
  // for TESS Disposition [2025-09-28: changed the columns from 10/11 to 11/12 to match ExoFOP change]
  const elsTESSDisp = Array.from(
    document.querySelectorAll(
      '#myGrid1 .ag-center-cols-container > div > div:nth-of-type(11)',
    ),
  );
  const isTESSHighlighted = highlightIfFalseAlarm(elsTESSDisp);
  // for TFOPWG Disposition
  const elsTFOPWGDisp = Array.from(
    document.querySelectorAll(
      '#myGrid1 .ag-center-cols-container > div > div:nth-of-type(12)',
    ),
  );
  ...
  const strTFOPWGDisp = elsTFOPWGDisp
    .map((e) => e.textContent.trim())
    .join(',');

  const strCombinedDisp =
    strTESSDisp || strTFOPWGDisp ? `${strTESSDisp} / ${strTFOPWGDisp}` : '';
  const styleExtras =
    isTESSHighlighted || isTFOPWGHighlighted
      ? 'background-color: rgba(255, 0, 0, 0.8);'
      : '';

  // Notes: [2025-09-28: changed the column from 16 to 17 to match ExoFOP change]
  const elsNotes = Array.from(
    document.querySelectorAll(
      '#myGrid1 .ag-center-cols-container > div > div:nth-of-type(17)',
    ),
  );
  ...
  document
    .querySelector('a[name="tois"] ~ div.grid_header')
    ?.insertAdjacentHTML(
      'beforeend',
      `
...
(() => {
  ...
  if (getAliasesList().some((alias) => alias.match(/EPIC|K2|Kepler|WASP/))) {
    const headerEl = document.querySelector(
      '.overview_header > span:first-of-type > span.purple-text',
    );
    headerEl?.insertAdjacentHTML(
      'afterend',
      `<span style="background-color: yellow; padding: 1px 2ch;"> Kepler/K2 </span>`,
    );
  }
})();
...
function showEpochInBTJDAndRelative() {
  const headerEl = document.querySelector(
    '#myGrid3 div.ag-header-container > div > div:nth-of-type(2)  span[ref="eText"]',
  );
  headerEl.textContent = 'Epoch (BTJD)'; // new header

  // actual conversion cell by cel
  const cellEpochs = document.querySelectorAll(
    '#myGrid3 .ag-center-cols-container > div > div:nth-of-type(2)',
  );
...
if (coord) {
  ...
  const headerEl = document.querySelector(
    `.overview_header > span:nth-of-type(${coord.uiCtrIdx}) > span.purple-text`,
  );
  headerEl.insertAdjacentHTML(
    'afterend',
    '<button id="raDecCopyCtl" style="font-size: 80%; margin-left: 6px;">Copy</button>',
  );
...
`,
  ...
  setTimeout(() => {
    document.getElementById('tempMsgPopIn')?.remove();
  }, durationInSec * 1000);
  ...
  function getTextToCopy(cellEl) {
    ...
    const masterPriority = (() => {
      try {
        ...
        return tableEl.nextElementSibling.querySelector(
          `div[row-index="${rowIdx}"] .ag-cell-value`,
        ).textContent;
      } catch (err) {
